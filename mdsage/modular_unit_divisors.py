...
def min_formula(N, t):
    ...
    return sum(
        N * Phi(gcd(i, N)) * (min(t, i / N) - 4 * (i / N) * (1 - i / N) * t)
        for i in srange(1, (N - 1) // 2 + 1)
    )
...
def divisor_F_bc(N, k):
    ...
    k = ZZ(k)
    return vector(
        [min_formula(k, i / N) * inverse_gcd(i, N) for i in srange(0, N // 2 + 1)]
    )
...
def conjectural_cuspidal_classgroup(N, degree_0_part=False):
    ...
    cusp_div = ZZ ** (N // 2 + 1)
    if degree_0_part:
        cusp_div = cusp_div.span(
            [
                cusp_div.gen(i) - degree_cusp(i, N) * cusp_div.gen(1)
...
def cusp_sums_from_signature_to_standard_basis(cuspsums, signatures, level):
    ...
    """
    P = Permutation(
        [cusp_number_from_signature(s, level) + 1 for s in signatures]
    ).inverse()
...
def diamond_permutation(list, d, level):
    N = len(list)
    return [
        list[(i * d) % level if (i * d) % level < N else (-i * d) % level]
        for i in range(N)
    ]
...
def diamond_orbits_cuspsum(list, level):
    return [
        diamond_permutation(list, n, level)
        for n in range(1, level // 2 + 1)
        if gcd(n, level) == 1
    ]
...
def split_into_diamond_orbits(cuspsums, level):
    ...
    diamonds = counts(
        [
            tuple(
                sorted(
                    set(csp_grp(vector(i)) for i in diamond_orbits_cuspsum(c, level))
                )
            )
            for c in cuspsums
        ]
